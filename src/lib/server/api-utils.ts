 ...
 */
export function responseCatchedError(error: unknown, defaultMessage: string, request?: NextRequest, additionalInfo?: Record<string, any>, title?: string): NextResponse {
    logError(error, additionalInfo, request, 'B', title); // 에러 로깅 (파일+터미널 둘다)
    const errorMessage = error instanceof Error ? error.message : defaultMessage; // 에러 메시지 추출
    const statusCode = error instanceof AuthenticationError ? 401 : 500;
    return NextResponse.json({ success: false, message: errorMessage, error: String(error) }, { status: statusCode });
}
...
export async function safeJsonParse<T>(request: Request, defaultValue: T): Promise<T> { // 타입안정성을 위해 제네릭을 사용하는 전형적인 예시임
    try { // 예) const body = await safeJsonParse(request, { userid: '', password: '' });
...
export function getClientIP(request?: NextRequest): string {
    ...
    const forwarded = request.headers.get('x-forwarded-for'); // 프록시 헤더 확인
    if (forwarded) {
...
export function getCallerFunctionName(): string {
    ...
    const helperFunctions = ['responseCatchedError', 'logError', 'logInfo', 'getCallerFunctionName', 'getClientIP']; // 헬퍼 함수 리스트
    let fallbackResult: string | null = null;
    for (let i = 1; i < stack.length; i++) { // 스택을 탐색하여 실제 호출자 찾기
        ...
        if (match) {
            ...
            const srcIndex = filePath.indexOf('src'); // src 이후 경로만 추출
            if (srcIndex !== -1) {
                let relativePath = filePath.substring(srcIndex + 4); // 'src/' 제거
                relativePath = relativePath.replace(/^[\\\/]+/, '').replace(/[\\\/]+/g, '>'); // 백슬래시를 >로 변경
                let cleanFuncName = funcName; // 함수명 정리 (Object.<anonymous> 등 제거)
                if (funcName.includes('.')) {
                    const parts = funcName.split('.');
                    cleanFuncName = parts[parts.length - 1];
                }                
                if (cleanFuncName === '<anonymous>' || cleanFuncName === 'anonymous') { // <anonymous>는 함수명에서 제거
                    if (!fallbackResult) { // fallback으로 파일명만 저장
                        fallbackResult = relativePath;
                    }
                    continue;
                }
                return `${relativePath}>${cleanFuncName}`;
            } else if (filePath) { // src가 없어도 파일 경로가 있으면 마지막 폴더/파일명만 추출
